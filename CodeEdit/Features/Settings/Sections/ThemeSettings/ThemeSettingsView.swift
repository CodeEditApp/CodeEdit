...
private extension ThemeSettingsView {
    ...
    private var changeThemeOnSystemAppearance: some View {
        Toggle(
            ...
            isOn: $prefs.settings.theme.mirrorSystemAppearance
        )
        .onChange(of: prefs.settings.theme.mirrorSystemAppearance) { value in
            if value {
                if colorScheme == .dark {
                ...
                themeModel.selectedTheme = themeModel.themes.first {
                    $0.name == prefs.settings.theme.selectedTheme
                }
            }
        }
    }
