...
final class ThemeModel: ObservableObject {
        ...
        didSet {
            DispatchQueue.main.async {
                SettingsModel.shared
                    .settings.theme.selectedLightTheme = self.selectedLightTheme?.name ?? "Broken"
            }
        }
    }
    ...
    var selectedDarkTheme: Theme? {
        didSet {
            DispatchQueue.main.async {
                SettingsModel.shared
                    .settings.theme.selectedDarkTheme = self.selectedDarkTheme?.name ?? "Broken"
            }
        }
    }
    ...
    var selectedTheme: Theme? {
        didSet {
            DispatchQueue.main.async {
                SettingsModel.shared.settings.theme.selectedTheme = self.selectedTheme?.name
            }
    ...
    func loadThemes() throws { // swiftlint:disable:this function_body_length
        ...
        let prefs = SettingsModel.shared.settings
    ...
    /// This function stores  'dark' and 'light' themes into `ThemeSettings` if user happens to select a theme
    ...
    func reset(_ theme: Theme) {
        SettingsModel.shared.settings.theme.overrides[theme.name] = [:]
        do {
            try self.loadThemes()
        } catch {
            print(error)
    ...
    func delete(_ theme: Theme) {
        ...
        do {
            ...
            SettingsModel.shared.settings.theme.overrides.removeValue(forKey: theme.name)
    ...
    private func saveThemes() {
        ...
        themes.forEach { theme in
            do {
                ...
                DispatchQueue.main.async {
                    SettingsModel.shared.settings.theme.overrides[theme.name] = newAttr
                }
