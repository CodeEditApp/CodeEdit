...
private extension AccountSettingsView {
    ...
    private var accountsSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack(spacing: 1) {
                ...
                if prefs.settings.accounts.sourceControlAccounts.gitAccount.isEmpty {
                    emptyView
                } else if
                    !prefs.settings.accounts.sourceControlAccounts.gitAccount.isEmpty && accountSelection == nil {
                    selectAccount
                } else {
                    accountTypeView
                }
    ...
    private var accountSelectionView: some View {
        VStack(alignment: .leading, spacing: 1) {
            ...
            List(
                $prefs.settings.accounts.sourceControlAccounts.gitAccount,
    ...
    private var accountTypeView: some View {
        VStack(alignment: .leading, spacing: 1) {
            ...
            VStack {
                ...
                SettingsSection("Clone Using", width: 100) {
                    let account = getSourceControlAccount(selectedAccountId: accountSelection ?? "")
                ...
                SettingsSection("SSH Key", width: 100) {
                    Picker("", selection: $prefs.settings.accounts.sourceControlAccounts.sshKey) {
                        Text("None")
                        Text("Create New...")
                        Text("Choose...")
                    }
    ...
    private func removeSourceControlAccount(selectedAccountId: String) {
        var gitAccounts = prefs.settings.accounts.sourceControlAccounts.gitAccount
