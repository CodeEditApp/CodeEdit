...
struct TabBarItemView: View {
    ...
    func closeAction() {
        isAppeared = false
        withAnimation(
            .easeOut(duration: prefs.settings.general.tabBarStyle == .native ? 0.15 : 0.20)
        ) {
    ...
    var content: some View {
        HStack(spacing: 0.0) {
            TabDivider()
                .opacity(
                    ...
                    && prefs.settings.general.tabBarStyle == .xcode ? 0.0 : 1.0
                )
                .padding(.top, isActive && prefs.settings.general.tabBarStyle == .native ? 1.22 : 0)
            ...
            HStack(alignment: .center, spacing: 5) {
                Image(systemName: item.systemImage)
                    ...
                    .foregroundColor(
                        prefs.settings.general.fileIconStyle == .color
            ...
            .frame(
                ...
                maxWidth: prefs.settings.general.tabBarStyle == .native ? .infinity : nil,
                // To max-out the parent (tab bar) area.
                maxHeight: .infinity
            )
            .padding(.horizontal, prefs.settings.general.tabBarStyle == .native ? 28 : 23)
            .overlay {
                ZStack {
                    if isActive {
                        // Close Tab Shortcut:
            ...
            .opacity(
                ...
                : (
                    isActive
                    ? (prefs.settings.general.tabBarStyle == .xcode ? 0.6 : 0.35)
                    : (prefs.settings.general.tabBarStyle == .xcode ? 0.4 : 0.55)
                )
            )
            TabDivider()
                .opacity(
        ...
        .overlay(alignment: .top) {
            ...
            TabBarTopDivider()
                .opacity(prefs.settings.general.tabBarStyle == .native && !isActive ? 1 : 0)
        }
        .foregroundColor(
            isActive && isActiveTabGroup
            ? (
                prefs.settings.general.tabBarStyle == .xcode && colorScheme != .dark
                ? Color(nsColor: .controlAccentColor)
                : .primary
            )
            : (
                prefs.settings.general.tabBarStyle == .xcode
                ? .primary
                : .secondary
            )
        )
    ...
    var body: some View {
        Button(action: switchAction) {
            ...
            .background {
                if prefs.settings.general.tabBarStyle == .xcode {
        ...
        .padding(
            ...
            .top, prefs.settings.general.tabBarStyle == .xcode ? 1 : 0
        )
//        .offset(
//            x: isAppeared || prefs.settings.general.tabBarStyle == .native ? 0 : -14,
//            y: 0
//        )
//        .opacity(isAppeared && onDragTabId != item.id ? 1.0 : 0.0)
        .zIndex(
            isActive
            ? (prefs.settings.general.tabBarStyle == .native ? -1 : 2)
            : (isDragging ? 3 : (isPressing ? 1 : 0))
        )
        .frame(
            width: (
                // Constrain the width of tab bar item for native tab style only.
                prefs.settings.general.tabBarStyle == .native
