...
extension FileSystemClient {
    ...
    final class FileSystemItem: Codable, Comparable, Hashable, Identifiable, TabBarItemRepresentable {
        ...
        var watcherCode: ((FileSystemItem) -> Void)?
        ...
        var doesExist: Bool { FileSystemItem.fileManger.fileExists(atPath: self.url.path) }
        ...
        required init(from decoder: Decoder) throws {
            ...
            children = try values.decode([FileSystemItem]?.self, forKey: .children)
        ...
        private func folderIcon(_ children: [FileSystemItem]) -> String {
            if self.parent == nil {
        ...
        /// Flattens the children of `FileSystemItem` recursively with depth.
        /// - Parameters:
        ///   - depth: An int
        ///   - ignoringFolders: <#ignoringFolders description#>
        /// - Returns: <#description#>
        func flattenedChildren(withDepth depth: Int, ignoringFolders: Bool) -> [FileSystemItem] {
            guard depth > 0 else { return [] }
            guard self.isFolder else { return [self] }
            var childItems: [FileSystemItem] = ignoringFolders ? [] : [self]
        ...
        /// Returns a list of `FileSystemItems` that are sibilings of ``self``. The `height` parameter lets te function navigate up the
        /// folder hierarchy to select a starting point from which it should start flettening the items.
        /// - Parameters:
        ...
        func flattenedSiblings(withHeight height: Int, ignoringFolders: Bool) -> [FileSystemItem] {
        ...
        private func getParent(withHeight height: Int) -> FileSystemItem {
        ...
        func childrenSatisfying(searchString: String, ignoredStrings: [String] = [".", "~"]) -> [FileSystemItem] {
            var satisfyingChildren: [FileSystemItem] = []
        ...
        func addFolder(folderName: String) {
            ...
            var fileNumber = 0
            while FileSystemItem.fileManger.fileExists(atPath: folderUrl.path) {
                fileNumber += 1
            ...
            // Create the folder
            do {
                try FileSystemItem.fileManger.createDirectory(
                    at: folderUrl,
        ...
        func addFile(fileName: String) {
            ...
            // If a file/folder with the same name exists, add a number to the end.
            var fileNumber = 0
            while FileSystemItem.fileManger.fileExists(atPath: fileUrl.path) {
                fileNumber += 1
            ...
            // Create the file
            FileSystemItem.fileManger.createFile(
                atPath: fileUrl.path,
                contents: nil,
        ...
        func delete() {
            ...
            if deleteConfirmation.runModal() == .alertFirstButtonReturn { // "Delete" button
                if FileSystemItem.fileManger.fileExists(atPath: self.url.path) {
                    do {
                        try FileSystemItem.fileManger.removeItem(at: self.url)
                    } catch {
                        fatalError(error.localizedDescription)
                    }
                }
...
//            Log.info("Duplicating file to \(fileUrl)")
            ...
            if FileSystemItem.fileManger.fileExists(atPath: self.url.path) {
                do {
                    try FileSystemItem.fileManger.copyItem(at: self.url, to: fileUrl)
                } catch {
//                    Log.error("Error at \(self.url.path) to \(fileUrl.path)")
        ...
        func move(to newLocation: URL) {
            guard !FileSystemItem.fileManger.fileExists(atPath: newLocation.path) else { return }
...
//                Log.info("Moving file \(self.url.debugDescription) to \(newLocation.debugDescription)")
                try FileSystemItem.fileManger.moveItem(at: self.url, to: newLocation)
            ...
            func createMissingParentDirectory(for url: URL, createSelf: Bool = true) {
                ...
                if !FileSystemItem.fileManger.fileExists(atPath: url.deletingLastPathComponent().path) {
                ...
                if createSelf && !FileSystemItem.fileManger.fileExists(atPath: url.path) {
//                    Log.info("Creating folder \(url.debugDescription)")
                    // Create the folder
                    do {
                        try FileSystemItem.fileManger.createDirectory(at: url,
                                                                withIntermediateDirectories: true,
                                                                attributes: [:])
                    } catch {
                        fatalError(error.localizedDescription)
        ...
        static func == (lhs: FileSystemClient.FileSystemItem, rhs: FileSystemClient.FileSystemItem) -> Bool {
            lhs.id == rhs.id
        }
